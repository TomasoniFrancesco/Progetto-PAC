...
async function avviaServer() {
    ...
    while (tentativi < 10) {
        try {
            ...
            // Migrazione: autenticazione Admin (Iterazione 3).
            // Garantisce l'esistenza dell'utente con ruolo 'admin' e ne imposta
            // la password di default (1111) finché l'hash è ancora il placeholder
            // dello schema di esempio. Una password già personalizzata NON viene
            // sovrascritta. La password di default è configurabile via env.
            try {
                const passwordDefault = process.env.ADMIN_PASSWORD_DEFAULT || '1111';
                const hashDefault = await authService.hashPassword(passwordDefault);
                const scritto = await utenti.assicuraAdmin(hashDefault);
                if (scritto) {
                    console.log('Migrazione auth: password admin impostata al valore di default');
                }
            } catch (errMigrazione) {
                console.log('Migrazione auth admin:', errMigrazione.message);
            }
